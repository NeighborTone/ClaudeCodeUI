...
- **複雑なタスク実施時**: `.claude/plan/task` ディレクトリに計画書を作成
- **Plan Mode使用時**: 作成した計画書を `.claude/plan/task` に保存
...
| 指定方法 | 値 | 用途 |
|----------|-----|------|
| **エイリアス（推奨）** | `sonnet` | バランス型、一般的な開発作業 |
| | `opus` | 複雑な推論・設計判断 |
| | `haiku` | 高速・低コスト、単純タスク |
...
## Workflow
This skill launches the `ue-explore` subagent to:
1. Search local UE source code (if configured)
2. Search official documentation
3. Return version-appropriate results
...
## Agent Invocation
```
Task(subagent_type="ue-explore", prompt="[user query]")
```
```
...
## Workflow
This skill launches the `unity-explore` subagent to:
1. Search installed package source
2. Search official Unity documentation
3. Return version-appropriate results
...
## Agent Invocation
```
Task(subagent_type="unity-explore", prompt="[user query]")
```
```
...
## Workflow
This skill launches the `[library]-explore` subagent to:
1. Search local library source (if available)
2. Search official documentation
3. Return version-appropriate results

## Agent Invocation
```
Task(subagent_type="[library]-explore", prompt="[user query]")
```
```
